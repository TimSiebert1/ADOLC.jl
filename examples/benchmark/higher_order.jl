...
function plot_uni_vs_tensor_eval_dir(suite, fixed_dim, fixed_d, directions)
    for experiment in keys(suite)
        ...
        p = Plots.plot()
        time_vals = [suite[experiment][fixed_dim][fixed_d][p] for p in directions]
        Plots.plot!(
            p,
            directions,
...
function plot_uni_vs_tensor_eval_deriv(suite, fixed_dim, derivative_orders, fixed_p)
    for experiment in keys(suite)
        base_times = [
            suite[experiment]["univariate"][fixed_dim][d] for d in derivative_orders
        ]
        binomis = [binomial(fixed_p + d - 1, d) for d in derivative_orders]
        time_vals = [suite[experiment][fixed_dim][d][fixed_p] for d in derivative_orders]
        p = Plots.plot()
        Plots.plot!(
            p,
            derivative_orders,
...
function run_mode(experiment, dim, derivative_order, directions; mode=:tensor_eval)
    ...
    elseif mode === :tensor_eval
        ...
        CxxTensor = ADOLC.myalloc2(
            m, binomial(directions + derivative_order, derivative_order)
        )

        seed = ADOLC.create_cxx_identity(dim, directions)

        time = @benchmark tensor_eval(
            0, $m, $dim, $derivative_order, $directions, $x, $CxxTensor, $seed
        )
    ...
    for experiment in experiments
        ...
        for dim in dims
            ...
            for d in derivative_orders
                ...
                suite[experiment]["univariate"][dim][d] = run_mode(
                    experiment, dim, d, 0; mode=:univariate
                )
                for p in directions
                    println("""
