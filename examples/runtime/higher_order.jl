    ...
    for experiment in keys(suite)
        ...
        for fixed_dim in fixed_dims
            for fixed_d in fixed_ds
                p = Plots.plot()
                directions_sorted =
                    sort(collect(keys(suite[experiment][fixed_dim][fixed_d])))
                base_time = suite[experiment]["base_time"][fixed_dim]
                time_vals =
                    [suite[experiment][fixed_dim][fixed_d][p] for p in directions_sorted]
                Plots.plot!(
                    p,
                    directions_sorted,
                    time_vals / base_time,
                    label = "n=$fixed_dim, d=$fixed_d",
                    legendfontsize = 5,
                )
                Plots.plot!(
                    p,
...
function run_mode(experiment, dim, derivative_order, num_directions)
    ...
    CxxTensor =
        ADOLC.myalloc2(m, binomial(num_directions + derivative_order, derivative_order))
